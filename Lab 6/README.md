...
```plantuml
@startuml
title Telepresence Emotion Cubes — System Architecture
...
' === LAYERS ===
rectangle "Emotion Cube\n(Raspberry Pi + OpenCV + NeoPixel)" as Cube {
    component "Camera\n(OpenCV + FER)" as Cam
    component "Emotion Detector\n(FER mtcnn=False)" as Detect
    component "NeoPixel LED Ring\nFade Transition" as LED
    component "MQTT Publisher\n(paho-mqtt → cube/{id}/emotion)" as Pub
}

rectangle "MQTT Broker\n(Mosquitto @ 10.56.129.182)" as Broker {
    component "Topic Routing\ncube/1/emotion, cube/2/emotion, cube/3/emotion" as Topics
}

rectangle "Flask Server\n(flask_mqtt + /colors API)" as Flask {
    component "MQTT Subscriber" as Sub
    component "Color Blender\n(Averages cube RGB values)" as Blend
    component "/colors Endpoint\n(JSON response)" as API
}

rectangle "Web Dashboard\n(index.html + JavaScript)" as Web {
    component "JS Fetch Loop\n(polls /colors every 500ms)" as JS
    component "Grid UI\n(Cube 1 | Cube 2 | Cube 3 | Blend)" as Grid
}

' === CONNECTIONS ===
U -[#gray]-> Cam : expresses emotion
Cam --> Detect : video frames
Detect --> LED : RGB color mapping
Detect --> Pub : publish RGB
LED -[#gray]-> U : diffused light feedback
Pub --> Topics : MQTT publish
Topics --> Sub : MQTT subscribe
Sub --> Blend : update color data
Blend --> API : provide JSON
API --> JS : send color data
JS --> Grid : update view
Grid -[#gray]-> U : on-screen feedback
